<template>
	<view class="wrapper">
        ...
        /> -->
				...
				/>
				<!-- 充值NT -->
				<RechargeNT v-if="type === 'recharge' && rechargeType === 'NT'" :amount.sync="amount" />
				<!-- 兑换 -->
				...
				<view class="tips" v-show="amount && type === 'purchase'">需扣除USDT：{{ amount }}</view>
				<view class="tips" v-show="amount && type === 'exchange'">需扣除配置U：{{ Number(amount * 1.2).toFixed(2) }}
				</view>
			</view>
			<view class="button">
				<u-button text="提交" size="large" color="linear-gradient(180deg, #D7AE73 0%, #DBC395 100%)"
					@click="transaction">
				</u-button>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			rechargeUsdt() {
				...
				let swapamount = BigNumber(this.amount)
					.times(1e18)
					.toFixed(0);
		...
		onLoad(options) {
			Object.assign(this, {
				...
				type: options.type && Object.keys(this.typeMap).includes(options.type) ?
					options.type :
					this.type,
			});
		},
